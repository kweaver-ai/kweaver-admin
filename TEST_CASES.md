...
| 6.6 | `KA auth list` | Lists every saved platform; `*` marks the active one; `status` is `valid` for a fresh login; `expires` is an ISO timestamp |
| 6.6a | `KA --json auth list` | JSON `{ currentPlatform, platforms: [{platform, active, username?, userId?, expiresAt?, status, refreshable, tlsInsecure}] }` |
| 6.6b | `KA auth ls` | Same output as `auth list` (alias) |
| 6.6c | After `auth logout` on the active platform, run `KA auth list` | The previously active platform is no longer listed (its `token.json` was removed) |
...
vars between rows if a case says ‚??cleared‚?ù.
