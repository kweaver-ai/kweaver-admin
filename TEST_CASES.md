...
ISF deployment (default target: `https://115.190.186.186`). Run in order;
each section assumes the previous one passed.

## Conventions

```bash
# Build once, then alias the binary for convenience.
npm run build
KA="node ./dist/index.js"
...
Capture results inline (✅ / ❌ / N/A) and any anomalies. Keep destructive
sections (2 / 4 / 6 / 9) self-cleaning.
...
| 1.5 | `$KA org get -- -1` | Same as above, exercises `-1` arg via `--` separator |
| 1.6 | `$KA org members <root-id>` | User table |
| 1.7 | `$KA role list` | 3 roles: 数据管理员 / AI管理员 / 应用管理员 |
...
| # | Command | Expectation |
|---|---|---|
| 6.1 | `$KA auth login --base-url https://115.190.186.186 -k` | Browser callback or device-code flow completes |
...
| 7.2 | `$KA -k --base-url https://115.190.186.186 org list` | Ad-hoc base url + insecure mode |
...
| 8.2 | `$KA config set baseUrl https://115.190.186.186` | Persists to file |
...
| # | Command | Expectation |
|---|---|---|
| 10.1 | `npm ci && npm run lint` | 0 errors |
| 10.2 | `npm test` | 12 files / 37 tests pass |
| 10.3 | `npm run build` | `dist/index.js` ≈135 KB, shebang `#!/usr/bin/env node` |
| 10.4 | `npm pack --dry-run` | Tarball contains only `dist/` + `README*` + `docs/SECURITY.md` + `package.json` |
| 10.5 | `node ./dist/index.js --version` | `0.6.0` |
