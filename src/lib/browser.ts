...
function isWsl(): boolean {
  ...
  if (process.platform !== "linux") return (cachedIsWsl = false);
  if (/microsoft/i.test(release())) return (cachedIsWsl = true);
  try {
    const procVersion = readFileSync("/proc/version", "utf8");
    return (cachedIsWsl = /microsoft/i.test(procVersion));
  } catch {
    return (cachedIsWsl = false);
  }
}
